define([
	...
	var app = angular.module('owloo', ['ngRoute', 'ngAnimate', 'ngCookies', 'factories', 'controllers', 'directives', 'filters', 'pascalprecht.translate']);
	...
	app.config (function ($routeProvider, $locationProvider) {
		...
		$routeProvider
			...
			.when('/', {
				...
				controller: 'HomeCtrl',
				resolve: {
					data: function ($q, OwlooUsers, FacebookPages) {
						// parameters
						var idiom = 'es';
						// factories
						var totalOwlooUsers  = OwlooUsers.getTotal().then(function (response) { return response.data; });
						var rankPageGrow = FacebookPages.getRankGrow(idiom).then(function (response) { return response.data; });
						// return
						return $q.all([totalOwlooUsers, rankPageGrow]).then(function(result) {
							return {
								'totalOwlooUsers': result[0],
								'rankPageGrow': result[1]
							}
						});
					}
				}
			...
			.when('/facebook-stats', {
				pageKey: 'facebook-stats',
				templateUrl: 'app/partials/facebook-stats.html',
				controller: 'FacebookStatsCtrl'
			})

			.when('/facebook-stats/pages/world/:category?/:page?', {
				pageKey: 'facebook-stats-pages-world',
				templateUrl: 'app/partials/facebook-stats-pages.html',
				controller: 'FacebookStatsPagesCtrl',
				resolve: {
			...
			.when('/facebook-stats/pages/hispanic/:category?/:page?', {
				pageKey: 'facebook-stats-pages-hispanic',
				templateUrl: 'app/partials/facebook-stats-pages.html',
				controller: 'FacebookStatsPagesCtrl',
				resolve: {
					data: function ($q, $route, FacebookPages) {
			...
			.when('/facebook-stats/pages/country/:country_code/:category?/:page?', {
				pageKey: 'facebook-stats-pages-country',
				templateUrl: 'app/partials/facebook-stats-pages-country.html',
				controller: 'FacebookStatsPagesCountryCtrl',
				resolve: {
					data: function ($q, $route, FacebookPages) {
						// parameters
						...
						var total = FacebookPages.getTotal().then(function (response) { return response.data; });
						var countries = FacebookPages.getCountries().then(function (response) { return response.data; });
						var categories = FacebookPages.getCategories().then(function (response) { return response.data; });
						// return
						return $q.all([rank, total, countries, categories]).then(function(result) {
			...
			.when('/features', {
				...
				resolve: {
					data: function ($q, $route, FacebookPages, TwitterProfiles, InstagramProfiles, OwlooUsers) {
						// factories
						var facebookPages = FacebookPages.getTotal().then(function (response) { return response; });
						var twitterProfiles = TwitterProfiles.getTotal().then(function (response) { return response; });
						var instagramProfiles = InstagramProfiles.getTotal().then(function (response) { return response; });
	...
	app.run(function ($rootScope) {
		...
		$rootScope.urlRoot = 'http://owloo.com/dev/3.1/';
		$rootScope.imagesFolder = 'http://owloo.com/dev/3.1/app/img/';
	...
	app.config(function ($translateProvider) {
		...
		$translateProvider
			...
			.translations('es', {
				...
				'MOST_TWITTER_ACCOUNT_GROWTH': 'Cuentas con mayor crecimiento en Twitter',
				'MOST_FACEBOOK_PAGES_GROWTH': 'Páginas con mayor crecimiento en Facebook',
				'MOST_FACEBOOK_COUNTRIES_GROWTH': 'Tasa de crecimiento de Facebook en $1 por países',
				...
				//************ FACEBOOK PAGE *****************
				'FACEBOOK_PAGE_GREYBAR1': 'Resumen de las estadísticas',
				...
				'FACEBOOK_PAGE_GREYBAR2': 'Crecimiento de fans durante los últimos $1 días',
				...
				'FACEBOOK_PAGE_GREYBAR3': 'Engagement rate de las publicaciones',
				...
				'FACEBOOK_PAGE_GREYBAR4': 'Facebook market share',
				...
				'FACEBOOK_PAGE_GREYBAR5': 'Segmentación de fans por países y ciudades',
				...
				'FACEBOOK_PAGE_GREYBAR6': 'Crecimiento de fans en $1 durante los últimos 30 días',
