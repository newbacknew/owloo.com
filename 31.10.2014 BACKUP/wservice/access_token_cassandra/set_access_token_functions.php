...
function getAccessToken_code($email, $pass){
	...
	try{
		...
		curl_setopt($curl, CURLOPT_COOKIEJAR, '/home/owloo/public_html/wservice/access_token_cassandra/cookie/cookies.txt');
		curl_setopt($curl, CURLOPT_COOKIEFILE, '/home/owloo/public_html/wservice/access_token_cassandra/cookie/cookies.txt');			
...
function getAccessTokenExpireDate($access_token){
	$curl = curl_init();
	//curl_setopt($curl, CURLOPT_COOKIE, 'COOKIENAME=COOKIEVALUE;COOKINAME2=COOKIEVALUE2');
	curl_setopt($curl, CURLOPT_COOKIEJAR, '/home/owloo/public_html/wservice/access_token_cassandra/cookie/cook.txt');
	curl_setopt($curl, CURLOPT_COOKIEFILE, '/home/owloo/public_html/wservice/access_token_cassandra/cookie/cook.txt');
	curl_setopt($curl, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US) AppleWebKit/534.10 (KHTML, like Gecko) Chrome/8.0.552.224 Safari/534.10');			
	curl_setopt($curl, CURLOPT_FAILONERROR, 1);
...
function setAccessToken(){
	...
	foreach($accounts as $fila){
		...
		if(file_exists('/home/owloo/public_html/wservice/access_token_cassandra/cookie/cookies.txt')) unlink('/home/owloo/public_html/wservice/access_token_cassandra/cookie/cookies.txt');
