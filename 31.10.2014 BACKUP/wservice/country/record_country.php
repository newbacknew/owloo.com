...
while($fila = mysql_fetch_assoc($que)){
	...
	while($ban == 0){
		...
		while(!$check_audience){
			$numAudience = getNumAudience($fila["code"], "", $accessToken, $accountId, $pageId, $pageName);
			if(!$numAudience){
				nextAccessToken();
				$conexion = mysql_connect("localhost", "owloo_admin", "fblatamx244") or die(mysql_error());
				mysql_select_db("owloo_owloo", $conexion) or die(mysql_error());
		...
		while(!$check_audience){
			$numAudienceFemale = getNumAudience($fila["code"], "2", $accessToken, $accountId, $pageId, $pageName);
		...
		while(!$check_audience){
			$numAudienceMale = getNumAudience($fila["code"], "1", $accessToken, $accountId, $pageId, $pageName);
			if(!$numAudienceMale){
				nextAccessToken();
				$conexion = mysql_connect("localhost", "owloo_admin", "fblatamx244") or die(mysql_error());
				mysql_select_db("owloo_owloo", $conexion) or die(mysql_error());
