...
function nextAccessToken(){
    ...
    if($accessToken_data_index < count($accessToken_data)){
        ...
        if($nun_intentos < $max_nun_intentos){
            ...
            send_email('region_comportamiento_3_1', 'Owloo ERROR - Region MOBILE OS 3.1', 'ERROR en la captura de datos.', true, 'Access Token - Region MOBILE OS - ID = '.$_SERVER['argv'][1]);
        }
    }
}

function getNumAudience($regionCode, $gender, $key_comportamiento, $_accessToken, $_accountId, $_pageId, $_pageName){
    $numAudience = "";
    try{
        
        $datos = get_url_content('https://graph.facebook.com/act_'.$_accountId.'/reachestimate?access_token='.$_accessToken.'&accountId='.$_accountId.'&bid_for=["conversion"]&currency=USD&endpoint=/act_'.$_accountId.'/reachestimate&locale=es_LA&method=get&pretty=0&targeting_spec={"genders":['.$gender.'],"age_max":65,"age_min":13,"regions":[],"countries":[],"cities":[],"zips":[],"keywords":[],"interests":[],"connections":[],"friends_of_connections":[],"relationship_statuses":null,"interested_in":[],"college_years":[],"education_statuses":[],"locales":[],"user_adclusters":[],"behaviors":[{"id":"'.$key_comportamiento.'","name":""}],"user_os":[],"user_device":[],"wireless_carrier":[],"work_positions":[],"education_majors":[],"education_schools":[],"work_employers":[],"page_types":null,"geo_locations":{"countries":[],"cities":[],"regions":[{"key":"'.$regionCode.'"}],"zips":[]},"excluded_geo_locations":{"countries":[],"cities":[],"regions":[],"zips":[]}}');
        ...
        if(isset($datosarray2['users'])){
            ...
            if(isset($datosarray2['error'])){
                if($datosarray2['error']['code'] != 190){
                    send_email('country_3_1', 'Owloo ERROR - Region COMPORTAMIENTO 3.1', 'ERROR en la captura de datos.', true, 'Facebook error code - Region COMPORTAMIENTO - ID = '.$_SERVER['argv'][1].' - Code: '.$datosarray2['error']['code'].' - Mensaje: '.$datosarray2['error']['message']);
                }
            }
        }
...
if(!empty($next_id)){
    ...
    /*$query = 'SELECT id_region FROM facebook_record_region_comportamiento_3_1 WHERE id_region = $1 AND id_comportamiento = $2 AND date = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
    ...
    if(!$row = mysql_fetch_assoc($que)){*/
        ...
        while($ban == 0 && $cont < 5){
            ...
            if(is_numeric($numAudience)){
                ...
                $query = 'INSERT INTO facebook_record_region_comportamiento_3_1 VALUES($1, $2, $3, NOW());';
                ...
                send_email('region_comportamiento_3_1', 'Owloo ERROR - Region MOBILE OS - '.$next_id.' - 3.1', 'ERROR en la captura de datos.', true, 'Captura de datos - Region MOBILE OS - ID = '.$_SERVER['argv'][1].' => '.$numAudience);
            }
            $cont++;
        }
        ...
        /*$query = 'SELECT COUNT(*) cantidad FROM `facebook_record_region_comportamiento_3_1` WHERE date = DATE_FORMAT(now(), \'%Y-%m-%d\');'; 
        $que = db_query($query, array());
        
        if($fila = mysql_fetch_assoc($que)){
            if($fila['cantidad'] == ($total_comportamiento))
                send_email('region_comportamiento_3_1', 'Owloo EXITO - Region MOBILE OS 3.1', 'EXITO en la captura de datos.');
        }*/
