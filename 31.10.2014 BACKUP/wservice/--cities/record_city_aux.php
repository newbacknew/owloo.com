...
function informarErrorActividades(){
	...
	$mensaje = 'Se ha detectado un error en la ejecución del script de captura de ranking de ciudades (aux). Favor verificar estado.';
...
$query = "SELECT id_city FROM record_city WHERE date = ".$date_insert.";"; 
$que = mysql_query($query, $conexion) or die(mysql_error());
$ids_ya_registrados = "";
$first = true;
while($fila = mysql_fetch_assoc($que)){
	$ids_ya_registrados .= ($first?$first=false:',').$fila['id_city'];
}


$query = "  SELECT id_city, fc.name name, key_city, code FROM facebook_city fc JOIN country c ON fc.id_country = c.id_country WHERE id_city not in(".$ids_ya_registrados.");"; 
$que = mysql_query($query, $conexion) or die(mysql_error());
$sql_value = "";

/*$query = "SELECT id_city, fc.name name, key_city, code FROM facebook_city fc JOIN country c ON fc.id_country = c.id_country WHERE fc.id_city > 1209 AND fc.id_city <= 2000 order by fc.id_country, id_city;"; 
$que = mysql_query($query, $conexion) or die(mysql_error());
$sql_value = "";*/
...
while($fila = mysql_fetch_assoc($que)){
        ...
        while($ban == 0){
                ...
                $sql = "INSERT INTO record_city(id_city, date, total_user, total_female, total_male) VALUES (" . $fila['id_city'] . ", DATE_FORMAT(".$date_insert.",'%Y-%m-%d'), " . $sql_value . ");";
...
$query = "SELECT COUNT(*) cantidad FROM record_city WHERE date = DATE_FORMAT(".$date_insert.", '%Y-%m-%d');"; 
$que = mysql_query($query, $conexion) or die(mysql_error());
if($fila = mysql_fetch_assoc($que)){
