...
function informarErrorActividades(){
	...
	$mensaje = 'Se ha detectado un error en la ejecución del script de captura de ranking de ciudades (1-20). Favor verificar estado.';
...
$query = "SELECT id_city, fc.name name, key_city, code FROM facebook_city fc JOIN country c ON fc.id_country = c.id_country WHERE fc.id_country >= 1 AND fc.id_country <= 20 order by fc.id_country, id_city;"; 
