...
function nextAccessToken(){
	...
	if($accessToken_data_index < count($accessToken_data)){
		...
		if($nun_intentos < $max_nun_intentos){
			...
			send_email('city_3_1', 'Owloo ERROR - Ciudades 3.1', 'ERROR en la captura de datos.', true, 'Access Token - Ciudad - ID = '.$_SERVER['argv'][1]);
		}
	}
}

function getNumAudience($cityCode, $gender, $_accessToken, $_accountId, $_pageId, $_pageName){
	$numAudience = "";
	try{
		...
		if(isset($datosarray2['users'])){
            ...
            if(isset($datosarray2['error'])){
                if($datosarray2['error']['code'] != 190){
                    send_email('country_3_1', 'Owloo ERROR - City 3.1', 'ERROR en la captura de datos.', true, 'Facebook error code - City - ID = '.$_SERVER['argv'][1].' - Code: '.$datosarray2['error']['code'].' - Mensaje: '.$datosarray2['error']['message']);
                }
            }
        }
...
$total_city = $_SERVER['argv'][3];

if(!empty($next_id)){
    
    /*$query = 'SELECT id_city FROM facebook_record_city_3_1 WHERE id_city = $1 AND date = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
    ...
    if(!$row = mysql_fetch_assoc($que)){*/
    	...
    	while($ban == 0 && $cont < 5){
            ...
            if(is_numeric($numAudience) && is_numeric($numAudienceMale) && is_numeric($numAudienceFemale)){
                ...
                $query = "INSERT INTO facebook_record_city_3_1 VALUES($1, $2, $3, $4, NOW());";
                ...
                send_email('city_3_1', 'Owloo ERROR - Ciudad - '.$next_id.' - 3.1', 'ERROR en la captura de datos.', true, 'Captura de datos - Ciudad - ID = '.$_SERVER['argv'][1].' => '.$numAudience.' - '.$numAudienceFemale.' - '.$numAudienceMale);
            }
            $cont++;
    	}
        ...
        /*$query = "SELECT COUNT(*) cantidad FROM `facebook_record_city_3_1` WHERE date = DATE_FORMAT(now(), '%Y-%m-%d');"; 
        $que = db_query($query, array());
        if($fila = mysql_fetch_assoc($que)){
        	if($fila['cantidad'] == $total_city)
        		send_email('city_3_1', 'Owloo EXITO - Ciudad 3.1', 'EXITO en la captura de datos.');
        }*/
