...
function informarErrorActividades(){
	...
	$mensaje = 'Se ha detectado un error en la ejecución del script de captura de ranking de edades, idiomas y relaciones (aux). Favor verificar estado.';
...
$query = "SELECT id_country, code FROM country WHERE id_country NOT IN (SELECT id_country FROM record_country_for_age_language WHERE date = DATE_FORMAT(".$aux_fecha.", '%Y-%m-%d')) ORDER BY 1;"; 
...
while($fila = mysql_fetch_assoc($que)){
	...
	while($ban == 0){
		...
		$sql = "INSERT INTO record_country_for_age_language VALUES(null, ".$fila['id_country'].", ".$sql_value." ".$aux_fecha.");";
...
$query = "SELECT COUNT(*) cantidad FROM `record_country_for_age_language` WHERE date = DATE_FORMAT(".$aux_fecha.", '%Y-%m-%d');"; 
$que = mysql_query($query, $conexion) or die(mysql_error());
if($fila = mysql_fetch_assoc($que)){
