...
function informarErrorActividades(){
	...
	$mensaje = 'Se ha detectado un error en la ejecución del script de captura de ranking de edades, idiomas y relaciones (1-20). Favor verificar estado.';
...
function getNumAudience($code, $gender, $minage, $maxage, $relacion, $locales, $broad_age, $education, $_accessToken, $_accountId){
	$numAudience = "";
	try{
//viejo		$datos = file_get_contents('https://graph.facebook.com/act_'.$_accountId.'/reachestimate?endpoint=/act_'.$_accountId.'/reachestimate&accountId='.$_accountId.'&locale=es_LA&targeting_spec={"genders":['.$gender.'],"age_max":'.$maxage.',"age_min":'.$minage.',"broad_age":'.$broad_age.',"regions":[],"countries":["'.$code.'"],"cities":[],"zips":[],"radius":0,"keywords":[],"connections":[],"excluded_connections":[],"friends_of_connections":[],"relationship_statuses":'.$relacion.',"interested_in":[],"college_networks":[],"college_majors":[],"college_years":[],"education_statuses":['.$education.'],"locales":['.$locales.'],"work_networks":[],"user_adclusters":[]}&method=get&access_token='.$accessToken);
...
$query = "SELECT id_country, code FROM country WHERE id_country >= 1 AND id_country <= 20 ORDER BY 1;"; 
