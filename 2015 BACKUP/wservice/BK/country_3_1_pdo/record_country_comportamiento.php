...
if(!empty($next_id)){
    ...
    $query = 'SELECT id_country FROM facebook_record_country_comportamiento_3_1 WHERE id_country = $1 AND id_comportamiento = $2 AND date = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
    $result = db_query($query, array($next_id, $id_comportamiento));
    ...
    if(!$row = $result->fetch(PDO::FETCH_ASSOC)){
        ...
        while($ban == 0 && $cont < 5){
            ...
            while(!$check_audience){
                $numAudienceFemale = getNumAudience($next_code, "2", $key_comportamiento, $accessToken, $accountId, $pageId, $pageName);
            ...
            while(!$check_audience){
                $numAudienceMale = getNumAudience($next_code, "1", $key_comportamiento, $accessToken, $accountId, $pageId, $pageName);
                if(!$numAudienceMale){
                    nextAccessToken();
                }
                else
            ...
            if(is_numeric($numAudience) && is_numeric($numAudienceMale) && is_numeric($numAudienceFemale)){
                ...
                $query = 'INSERT INTO facebook_record_country_comportamiento_3_1 VALUES($1, $2, $3, $4, $5, NOW());';
                $values = array($next_id, $id_comportamiento, $numAudience, $numAudienceFemale, $numAudienceMale);
                ...
                send_email('country_comportamiento_3_1', 'Owloo ERROR - Pais COMPORTAMIENTO - '.$next_id.' - 3.1', 'ERROR en la captura de datos.', true, 'Captura de datos - Pais COMPORTAMIENTO - ID = '.$_SERVER['argv'][1].' => '.$numAudience.' - '.$numAudienceFemale.' - '.$numAudienceMale);
            }
            $cont++;
        }
