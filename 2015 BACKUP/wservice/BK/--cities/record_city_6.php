...
function informarErrorActividades(){
	...
	$mensaje = 'Se ha detectado un error en la ejecución del script de captura de ranking de ciudades (101-120). Favor verificar estado.';
...
$query = "SELECT id_city, fc.name name, key_city, code FROM facebook_city fc JOIN country c ON fc.id_country = c.id_country WHERE fc.id_country >= 101 AND fc.id_country <= 120 order by fc.id_country, id_city;"; 
