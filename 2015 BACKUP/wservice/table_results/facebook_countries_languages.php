<?php
    ...
    error_log('   Facebook Country Language (i): '.date('d m Y H:i:s'));
    
    require_once(__DIR__.'/../config.php');
    
    function get_country_languages_date_last_update($count){
        $sql = 'SELECT date 
                  FROM '.DB_FACEBOOK_PREFIX.'record_country_language_3_1
                  GROUP BY date 
                  HAVING count(date) = $1
                  ORDER BY 1 DESC 
                  LIMIT 1;
    ...
    $languages = array();
    $query = 'SELECT id_language, nombre FROM '.DB_FACEBOOK_PREFIX.'language_3_1 WHERE active_fb_get_data = 1 AND active = 1;';
    ...
    foreach($countries as $country){
        ...
        $query = 'SELECT id_language, total_user FROM '.DB_FACEBOOK_PREFIX.'record_country_language_3_1 WHERE id_country = $1 AND date = \'$2\';';
        ...
        while($audience = mysql_fetch_assoc($que_audience)){
            ...
            $query = 'SELECT id_language, country_code FROM '.DB_RESULTS_PREFIX.'facebook_countries_languages WHERE country_code = \'$10\' AND id_language = $11;';
            $que_language = db_query_table_results($query, array($country_code, $id_language));
            if($row = mysql_fetch_assoc($que_language)){
                $query = "UPDATE ".DB_RESULTS_PREFIX."facebook_countries_languages SET 
                                                    name = '$10',
                                                    total_user = $11,
                                                    grow_30 = $12,
                                                    updated_at = NOW()
                ...
                $query = "INSERT INTO ".DB_RESULTS_PREFIX."facebook_countries_languages VALUES(NULL, $10, '$11', '$12', $13, $14, NOW());";
                $values = array(
                                    $id_language,
                                    $name,
                                    $country_code,
    ...
    error_log('   Facebook Country Language (f): '.date('d m Y H:i:s'));