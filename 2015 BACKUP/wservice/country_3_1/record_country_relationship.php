...
function nextAccessToken(){
    ...
    if($accessToken_data_index < count($accessToken_data)){
        ...
        if($nun_intentos < $max_nun_intentos){
            ...
            send_email('country_relationship_3_1', 'Owloo ERROR - Pais RELATIONSHIP 3.1', 'ERROR en la captura de datos.', true, 'Access Token - Pais RELATIONSHIP - ID = '.$_SERVER['argv'][1]);
        }
    }
}
        ...
        if(isset($datosarray2['users'])){
            ...
            if(isset($datosarray2['error'])){
                if($datosarray2['error']['code'] != 190){
                    send_email('country_3_1', 'Owloo ERROR - Pais RELATIONSHIP 3.1', 'ERROR en la captura de datos.', true, 'Facebook error code - Pais - ID = '.$_SERVER['argv'][1].' - Code: '.$datosarray2['error']['code'].' - Mensaje: '.$datosarray2['error']['message']);
                }
            }
        }
...
if(!empty($next_id)){
    ...
    if($check_if_exist){
        $query = 'SELECT id_country FROM facebook_record_country_relationship_3_1 WHERE id_country = $1 AND id_relationship = $2 AND date = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
    ...
    while($ban == 0 && $cont < 5){
        ...
        if(is_numeric($numAudience) && is_numeric($numAudienceMale) && is_numeric($numAudienceFemale)){
            ...
            $query = 'INSERT INTO facebook_record_country_relationship_3_1 VALUES($1, $2, $3, $4, $5, NOW());';
            $values = array($next_id, $id_relationship, $numAudience, $numAudienceFemale, $numAudienceMale);
            ...
            send_email('country_relationship_3_1', 'Owloo ERROR - Pais RELATIONSHIP - '.$next_id.' - 3.1', 'ERROR en la captura de datos.', true, 'Captura de datos - Pais RELATIONSHIP - ID = '.$_SERVER['argv'][1].' => '.$numAudience.' - '.$numAudienceFemale.' - '.$numAudienceMale);
        }
        $cont++;
    }
    ...
    /*$query = 'SELECT COUNT(*) cantidad FROM `facebook_record_country_relationship_3_1` WHERE date = DATE_FORMAT(now(), \'%Y-%m-%d\');'; 
    $que = db_query($query, array());
    
    if($fila = mysql_fetch_assoc($que)){
        if($fila['cantidad'] == ($total_relationship))
            send_email('country_relationship_3_1', 'Owloo EXITO - Pais RELATIONSHIP 3.1', 'EXITO en la captura de datos.');
    }*/
