<?php
    ...
    foreach($interests as $fila){
        ...
        exec('php-cli '.$folder_path.'record_country_interest.php '.$_SERVER['argv'][1].' '.$_SERVER['argv'][2].' '.($_SERVER['argv'][3]*$total_interest).' '.$fila['id'].' '.$fila['key'].' '.$_SERVER['argv'][6].' c45a5f3b2cfa74ac94bd5bbfb2c5d6a5 > '.$log_path.'record_country_interest.log 2>&1 &');   
