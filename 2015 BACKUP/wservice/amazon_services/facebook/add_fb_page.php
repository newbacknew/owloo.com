<?php
    ...
    error_log('   Add Page (i): '.date('d m Y H:i:s'));
                ...
                */
                ...
                if(!empty($id_page)){
                    ...
                    if(!is_exist_today_fb_page_likes_talking_about($id_page)){
                        add_fb_page_likes_talking_about($id_page, $datos['likes'], $datos['talking_about_count'], $datos['were_here_count']);
                    }
                    ...
                    error_log('   Add Page (f): '.date('d m Y H:i:s'));
                    
                    //Generamos y enviamos los datos
                    
                    error_log('   Generate Results (i): '.date('d m Y H:i:s'));
                    
                    generate_data_results($id_page);
                    
                }
                else {
                    $error = array('error_code' => 2, 'description' => 'Sorry, an error has occurred');
                }
            }
            else {
                $error = array('error_code' => 1, 'description' => 'Page not found');
            }
        }
        else {
            $error = array('error_code' => 1, 'description' => 'Page not found');
