<?php
    ...
    function get_country_total_count($hispanic = false){
        $sql = 'SELECT count(*) cantidad FROM country '.($hispanic?'WHERE habla_hispana = 1':'').';';
    ...
    function getCountryData($code){
        $sql =   "SELECT nombre, name, total_user, date, code, total_male, total_female
                        FROM record_country rc JOIN country c ON rc.id_country = c.id_country 
    ...
    function get_country_id_by_location($country){
        $sql = "SELECT id_country FROM country WHERE name LIKE '".mysql_real_escape_string($country)."';";
        $res = mysql_query($sql) or die(mysql_error());
        if($fila = mysql_fetch_assoc($res)){
            return $fila['id_country'];
        }
    ...
    function is_hispanic_country($id_country){
        $sql = "SELECT habla_hispana FROM country WHERE id_country = ".mysql_real_escape_string($id_country).";";
        $res = mysql_query($sql) or die(mysql_error());
        if($fila = mysql_fetch_assoc($res)){
            return $fila['habla_hispana'];
        }
        return 0;
    }
    ...
    function add_fb_page_likes_talking_about($id_page, $likes, $talking_about_count){
        if(!empty($id_page) && is_numeric($likes) && is_numeric($talking_about_count)){
            ...
            $sql = "INSERT INTO facebook_pages_likes_talking_about VALUES(NULL, $id_page, $likes, $talking_about_count, NOW());";
            $res = mysql_query($sql) or die(mysql_error());
            if(mysql_affected_rows() > 0){
                return mysql_insert_id();
            }
