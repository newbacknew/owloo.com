    ...
    if($accessToken_data_index < count($accessToken_data)){
        ...
        if($nun_intentos < $max_nun_intentos){
           /* setAccessToken();
            ...
            send_email('region_age_3_1', 'Owloo ERROR - Region AGE 3.1', 'ERROR en la captura de datos.', true, 'Access Token - Region AGE - ID = '.$_SERVER['argv'][1]);
        }
    }
}

function getNumAudience($regionCode, $age_min, $age_max, $gender, $_accessToken, $_accountId, $_pageId, $_pageName){
    $numAudience = "";
    try{
        ...
        if(isset($datosarray2['users'])){
            ...
            if(isset($datosarray2['error'])){
                if($datosarray2['error']['code'] != 190){
                    send_email('country_3_1', 'Owloo ERROR - Region AGE 3.1', 'ERROR en la captura de datos.', true, 'Facebook error code - Region AGE - ID = '.$_SERVER['argv'][1].' - Code: '.$datosarray2['error']['code'].' - Mensaje: '.$datosarray2['error']['message']);
                }
            }
        }
...
if(!empty($next_id)){
    ...
    /*$query = 'SELECT id_region FROM facebook_record_region_age_3_1 WHERE id_region = $1 AND id_age = $2 AND date = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
    ...
    if(!$row = mysql_fetch_assoc($que)){*/
        ...
        while($ban == 0 && $cont < 5){
            ...
            if(is_numeric($numAudience)){
                ...
                $query = 'INSERT INTO facebook_record_region_age_3_1 VALUES($1, $2, $3, NOW());';
                ...
                send_email('region_age_3_1', 'Owloo ERROR - Region AGE - '.$next_id.' - 3.1', 'ERROR en la captura de datos.', true, 'Captura de datos - Region AGE - ID = '.$_SERVER['argv'][1].' => '.$numAudience);
            }
            $cont++;
        }
        ...
        /*$query = 'SELECT COUNT(*) cantidad FROM `facebook_record_region_age_3_1` WHERE date = DATE_FORMAT(now(), \'%Y-%m-%d\');'; 
        $que = db_query($query, array());
        if($fila = mysql_fetch_assoc($que)){
            if($fila['cantidad'] == ($total_age_rows))
                send_email('region_age_3_1', 'Owloo EXITO - Region AGE 3.1', 'EXITO en la captura de datos.');
        }*/
