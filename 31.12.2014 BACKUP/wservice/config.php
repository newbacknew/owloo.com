<?php
    ...
    define('DB_TWITTER_PREFIX', 'owloo_');
    ...
    define('INSTAGRAM_CLIENT_ID', '881fc93c95074b7ca75818e3c6a1c4d8');
    define('INSTAGRAM_COUNT_DATA', 50);
    ...
    function db_query($sql, $values, $insert_update = false, $num_aux = 1){
        ...
        $count = $num_aux; foreach ($values as $value) {
    ...
    function db_query_tw($sql, $values, $insert_update = false){
        $conexion_owloo = mysql_connect('localhost', DB_USER, DB_PASS) or die(mysql_error());
    ...
    function register_today_success($type){
        $query = 'INSERT INTO `'.DB_FACEBOOK_PREFIX.'owloo_cron_send_success_3_1` VALUES(NULL, \'$1\', NOW());'; 
        $values = array($type);
        db_query($query, $values, 1);
    }
    
    function register_error($type, $error){
        $query = 'INSERT INTO `'.DB_FACEBOOK_PREFIX.'owloo_cron_error_info_3_1` VALUES(NULL, \'$1\', \'$2\', NOW());'; 
        $values = array($type, $error);
        db_query($query, $values, 1);
    }
    
    function is_sent_today_success($type){
        $query = 'SELECT * FROM `'.DB_FACEBOOK_PREFIX.'owloo_cron_send_success_3_1` WHERE type LIKE \'$1\' AND DATE_FORMAT(date, \'%Y-%m-%d\') = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
        $values = array($type);
    ...
    function is_sent_today_error($type){
        $query = 'SELECT * FROM `'.DB_FACEBOOK_PREFIX.'owloo_cron_error_info_3_1` WHERE type LIKE \'$1\' AND date = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
        $values = array($type);
        $que = db_query($query, $values);
        if($fila = mysql_fetch_assoc($que)){
            return true;
