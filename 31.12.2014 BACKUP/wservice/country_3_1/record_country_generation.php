        ...
        if(isset($datosarray2['users'])){
            ...
            if(isset($datosarray2['error'])){
                if($datosarray2['error']['code'] != 190){
                    send_email('country_3_1', 'Owloo ERROR - Pais GENERATION 3.1', 'ERROR en la captura de datos.', true, 'Facebook error code - Pais - ID = '.$_SERVER['argv'][1].' - Code: '.$datosarray2['error']['code'].' - Mensaje: '.$datosarray2['error']['message']);
                }
            }
        }
...
$total_generation = $_SERVER['argv'][3];
$id_generation = $_SERVER['argv'][4];
$key_generation = $_SERVER['argv'][5];
$check_if_exist = $_SERVER['argv'][6];

if(!empty($next_id)){
        
    if($check_if_exist){
        $query = 'SELECT id_country FROM facebook_record_country_generation_3_1 WHERE id_country = $1 AND id_generation = $2 AND date = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
        $que = db_query($query, array($next_id, $id_generation));
    ...
    while($ban == 0 && $cont < 5){
        ...
        if(is_numeric($numAudience)){
            ...
            $query = 'INSERT INTO facebook_record_country_generation_3_1 VALUES($1, $2, $3, NOW());';
            $values = array($next_id, $id_generation, $numAudience);
            ...
            send_email('country_generation_3_1', 'Owloo ERROR - Pais GENERATION - '.$next_id.' - 3.1', 'ERROR en la captura de datos.', true, 'Captura de datos - Pais GENERATION - ID = '.$_SERVER['argv'][1].' => '.$numAudience);
        }
        $cont++;
    }
    ...
    /*$query = 'SELECT COUNT(*) cantidad FROM `facebook_record_country_generation_3_1` WHERE date = DATE_FORMAT(now(), \'%Y-%m-%d\');';  
    $que = db_query($query, array());
    
    if($fila = mysql_fetch_assoc($que)){
        if($fila['cantidad'] == ($total_generation))
            send_email('country_generation_3_1', 'Owloo EXITO - Pais GENERATION 3.1', 'EXITO en la captura de datos.');
    }*/
