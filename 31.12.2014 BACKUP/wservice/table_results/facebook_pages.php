<?php
    ...
    function getHistory30Array($id_page, $fb_page_likes_last_update){
        ...
        $daily_likes_grow = array(
                ...
                'min_change_of_followers_on' => $daily_likes_grow_seriesDataMin,
                'max_change_of_followers_on' => $daily_likes_grow_seriesDataMax,
    ...
    while($fila = mysql_fetch_assoc($que)){
        $countries[$fila['id_country']] = array('id' => $fila['id_country'], 'code' => $fila['code'], 'name' => $fila['nombre'], 'slug' => $fila['name'], 'idiom' => $fila['idiom'],  'audience' => get_country_audience($fila['id_country'], $country_date_last_update));
    }
    ...
    //General ranking
    $query = 'SELECT id_page FROM '.DB_RESULTS_PREFIX.'facebook_pages WHERE parent = 0 ORDER BY likes DESC, talking_about DESC;';
