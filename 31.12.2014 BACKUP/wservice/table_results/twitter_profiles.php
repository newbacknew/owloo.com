<?php
    ...
    function get_current_picture_profile($username){
        ...
        if(isset($picture[0]->attr['src'])){
            ...
            return 'http://a0.twimg.com/sticky/default_profile_images/default_profile_4_200x200.png';
        }
    }
    ...
    function owlooFormatPorcent($number, $total = NULL, $decimal = 3, $sep_decimal = ',', $sep_miles = '.') {
        if ($total === 0)
            return 0;
        
        if(is_int($number) && $total === NULL){
            return $number;
        }
    ...
    function get_tweets_made_by_day($id_profile){
        $qry = "SELECT DISTINCT owloo_tweet_date, count(*) tweets
                FROM owoo_tweet_data
                WHERE owloo_user_id = $1 AND owloo_tweet_date >= '".get_twitter_date_for_days($id_profile, 30)."'
                GROUP BY owloo_tweet_date
                ORDER BY owloo_tweet_date ASC
                LIMIT 0 , 35";
    ...
    function get_tweets_made_by_day_of_the_week($id_profile){
        $qry = 'SELECT WEEKDAY( `owloo_tweet_date` ) day_of_week , count(*) tweets
                FROM `owoo_tweet_data`
                WHERE `owloo_user_id` = $1
                GROUP BY 1';
    ...
    function getMentionsArray($id_profile, $count = 10){
        $mentions = array();
        $qry = ' SELECT owloo_screenanme AS name, count(*) AS count
                    FROM '.DB_TWITTER_PREFIX.'mentions
                    WHERE owloo_user_id = $1
                    GROUP BY owloo_screenanme
                    ORDER BY count DESC
                    LIMIT 0 , $2;';
    ...
    function getHashtagsArray($id_profile, $count = 10){
        $hastags = array();
        $qry = ' SELECT owloo_hashword AS name, count(*) AS count
                    FROM '.DB_TWITTER_PREFIX.'hashtag
                    WHERE owloo_user_id = $1
                    GROUP BY owloo_hashword
                    ORDER BY count DESC
                    LIMIT 0 , $2;';
    ...
    $query = 'SELECT * FROM '.DB_TWITTER_PREFIX.'user_master WHERE owloo_user_status = 1 ORDER BY owloo_user_id;';
