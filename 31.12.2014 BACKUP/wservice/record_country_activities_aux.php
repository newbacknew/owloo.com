...
function informarErrorActividades(){
	...
	$mensaje = 'Se ha detectado un error en la ejecución del script de captura de ranking de actividades (aux). Favor verificar estado.';
...
$query = "SELECT id_country, code FROM country WHERE id_country NOT IN (SELECT id_country FROM record_country_for_user_preference WHERE date = DATE_FORMAT(".$aux_fecha.", '%Y-%m-%d')) ORDER BY 1;"; 
...
mysql_select_db("owloo_owloo", $conexion) or die(mysql_error());
		...
		$sql = "INSERT INTO record_country_for_user_preference VALUES(null, ".$fila['id_country'].", ".$aux_fecha." ".$sql_value.");";
...
$query = "SELECT COUNT(*) cantidad FROM `record_country_for_user_preference` WHERE date = DATE_FORMAT(".$aux_fecha.", '%Y-%m-%d');"; 
$que = mysql_query($query, $conexion) or die(mysql_error());
if($fila = mysql_fetch_assoc($que)){
