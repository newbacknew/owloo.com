    ...
    function cron_insert_likes_talking_about_from_fb($ids){
        ...
        if(!empty($datosarray)){
            ...
            foreach($datosarray as $page){
                ...
                $sql_insert .= "(NULL, ".mysql_real_escape_string($array_fb_id_to_id_page[$page['page_id']]).",".mysql_real_escape_string($page['fan_count']).",".mysql_real_escape_string($page['talking_about_count']).",NOW())";
                
                cron_update_fb_page_likes_talking($array_fb_id_to_id_page[$page['page_id']], $page['fan_count'], $page['talking_about_count']);
                
            }
            $sql = "INSERT INTO facebook_pages_likes_talking_about(id, id_page, likes, talking_about, date) VALUES".$sql_insert.";";
            $res = mysql_query($sql) or die(mysql_error());
        }
    }
