...
function nextAccessToken(){
    ...
    if($accessToken_data_index < count($accessToken_data)){
        ...
        if($nun_intentos < $max_nun_intentos){
            ...
            send_email('country_interest', 'Owloo ERROR - Pais INTEREST', 'ERROR en la captura de datos.', true, 'Access Token - Pais INTEREST - ID = '.$_SERVER['argv'][1]);
        }
    }
}
...
if(!empty($next_id)){
    ...
    $query = 'SELECT id_country FROM facebook_record_country_interest WHERE id_country = $1 AND id_interest = $2 AND date = DATE_FORMAT(NOW(), \'%Y-%m-%d\');';
    $que = db_query($query, array($next_id, $id_interest));
    
    if(!$row = mysql_fetch_assoc($que)){
        
    $row = cassandra_query("SELECT id_country FROM facebook_record_country_interest WHERE id_country = $next_id AND id_interest = $id_interest AND date = $date;");
    ...
    if(count($row) == 0){
        ...
        while($ban == 0){
            ...
            if(is_numeric($numAudience) && is_numeric($numAudienceMale) && is_numeric($numAudienceFemale)){
                ...
                cassandra_insert("INSERT INTO facebook_record_country_interest(id_country,id_interest,total_user,total_female,total_male,date,date_time) VALUES($next_id, $id_interest, $numAudience, $numAudienceFemale, $numAudienceMale, $date, dateof(NOW()));");
                $ban = 1;
                
            }
            else {
                send_email('country_interest', 'Owloo ERROR - Pais INTEREST - '.$next_id, 'ERROR en la captura de datos.', true, 'Captura de datos - Pais INTEREST - ID = '.$_SERVER['argv'][1].' => '.$numAudience.' - '.$numAudienceFemale.' - '.$numAudienceMale);
            }
        }
        ...
        $query = 'SELECT COUNT(*) cantidad FROM `facebook_record_country_interest` WHERE date = DATE_FORMAT(now(), \'%Y-%m-%d\');'; 
        $que = db_query($query, array());
        
        if($fila = mysql_fetch_assoc($que)){
            if($fila['cantidad'] == ($total_interest))
                send_email('country_interest', 'Owloo EXITO - Pais INTEREST 3.1', 'EXITO en la captura de datos.');
        }
