...
function nextAccessToken(){
    ...
    if($accessToken_data_index < count($accessToken_data)){
        ...
        if($nun_intentos < $max_nun_intentos){
            ...
            send_email('country_generation', 'Owloo ERROR - Pais GENERATION', 'ERROR en la captura de datos.', true, 'Access Token - Pais GENERATION - ID = '.$_SERVER['argv'][1]);
        }
    }
}
...
if(!empty($next_id)){
    ...
    $row = cassandra_query("SELECT id_country FROM facebook_record_country_generation WHERE id_country = $next_id AND id_generation = $id_generation AND date = $date;");
