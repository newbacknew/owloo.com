<?php
    ...
    foreach($ages as $fila){
        ...
        exec('php-cli '.$folder_path.'record_country_age.php '.$_SERVER['argv'][1].' '.$_SERVER['argv'][2].' '.($_SERVER['argv'][3]*$total_age).' '.$fila['id'].' '.$fila['min'].' '.$fila['max'].' c45a5f3b2cfa74ac94bd5bbfb2c5d6a5 > '.$log_path.'record_country_age.log 2>&1 &');
